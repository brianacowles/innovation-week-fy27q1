...
function setLoading(isLoading) {
  if (isLoading) {
    ...
    analyzeBtn.textContent = 'Run Survey';
  }
}
...
function renderSurveyFindings(findings) {
  ...
  fillList(observationsList, findings?.observations || []);
  fillList(opportunitiesList, findings?.opportunities || []);
  fillList(evidenceList, findings?.evidence_checklist || []);
}
...
function renderResults(data) {
  ...
  renderSurveyFindings(data.survey_findings || null);
...
analysisForm.addEventListener('submit', async (event) => {
  ...
  formData.append('survey_type', surveyType?.value || 'shelf');
